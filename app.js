...
function guidePage(){
  ...
  <div class="guide-load-progress" data-guide-load-progress ${guideLoading?'':'hidden'}><div><span data-guide-load-text>Loading programme guide…</span><strong data-guide-load-percent>0%</strong></div><i><b data-guide-load-bar style="width:0%"></b></i><small>Still working — programme data will appear channel by channel.</small></div>
...
async function loadGuideEpg(){
  ...
  guideLoading=true;guideError='';document.querySelectorAll('.guide-loading span').forEach(el=>el.textContent='Loading programme guide…');const guideProgress=document.querySelector('[data-guide-load-progress]'),guideBar=document.querySelector('[data-guide-load-bar]'),guidePct=document.querySelector('[data-guide-load-percent]'),guideText=document.querySelector('[data-guide-load-text]');if(guideProgress)guideProgress.hidden=false;let guideDone=channels.filter(ch=>Number(epgCache.get(ch.id)?.loadedAt||0)>=stale).length;const updateGuideProgress=(done,total,label='Loading programme guide…')=>{const pct=Math.max(2,Math.min(98,Math.round((Number(done||0)/Math.max(1,Number(total||1)))*100)));if(guideBar)guideBar.style.width=`${pct}%`;if(guidePct)guidePct.textContent=`${pct}%`;if(guideText)guideText.textContent=label};updateGuideProgress(0,channels.length,'Contacting TV guide sources…');
  try{
    ...
    for(const p of enabledProviders().filter(x=>x.type==='m3u'&&x.epgUrl&&!m3uGuideLoadedProviders.has(x.id))){
      ...
      try{const text=NATIVE_WINDOWS?await nativeFetchText(p.epgUrl):await (await fetch(p.epgUrl)).text();const wanted=new Set(providerChannels.map(c=>preferredLiveSource(c).tvgId||c.name).filter(Boolean));const parsed=parseXMLTV(text,wanted);for(const ch of providerChannels){const src=preferredLiveSource(ch),key=src.tvgId||ch.name;epgCache.set(ch.id,{loadedAt:Date.now(),list:parsed[key]||[]});updateGuideRow(ch);guideDone++;updateGuideProgress(guideDone,channels.length,`Loaded programme data for ${guideDone} of ${channels.length} channels…`)}m3uGuideLoadedProviders.add(p.id)}catch{}
    }
    const pending=channels.filter(ch=>!epgCache.get(ch.id)||epgCache.get(ch.id).loadedAt<stale);let cursor=0;
    const worker=async()=>{while(cursor<pending.length){const ch=pending[cursor++],src=preferredLiveSource(ch);if(src.source!=='xtream'){guideDone++;updateGuideProgress(guideDone,channels.length,`Checked ${guideDone} of ${channels.length} channels…`);continue}const cfg=providerConfigFor(src);try{if(!cfg.server||!cfg.username||!cfg.password)throw new Error('Missing saved login');const payload=await fetchXtreamShortEpg(cfg,src.streamId,12);epgCache.set(ch.id,{loadedAt:Date.now(),list:normalizeXtreamEpg(payload)});updateGuideRow(ch)}catch{epgCache.set(ch.id,{loadedAt:Date.now(),list:[]});updateGuideRow(ch)}guideDone++;updateGuideProgress(guideDone,channels.length,`Loaded programme data for ${Math.min(guideDone,channels.length)} of ${channels.length} channels…`)}};
  ...
  finally{guideLoading=false;if(guideBar)guideBar.style.width='100%';if(guidePct)guidePct.textContent='100%';if(guideText)guideText.textContent=guideError?'Guide load finished with limited data':'Programme guide ready';setTimeout(()=>{if(guideProgress)guideProgress.hidden=true},900);const alert=document.querySelector('.guide-alert');if(guideError&&!alert){const shell=document.querySelector('.guide-shell');if(shell)shell.insertAdjacentHTML('afterbegin',`<div class="guide-alert">${esc(guideError)}</div>`)}}
}
