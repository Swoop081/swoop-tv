...
function patchMountedHomeRows(ids=[]){
  ...
  const status=document.querySelector('.discovery-status');if(status){status.textContent=discoveryRefreshing?'Refreshing Swoop discovery…':discoveryMessage||'Trending refreshes about every 90 minutes · Top 100 every 4 hours';status.classList.toggle('busy',discoveryRefreshing)}
  return changed;
}
...
function home(){
  ...
  const discoveryNote=customNeedsKey?`<section class="web-discovery-callout"><div><span class="eyebrow">CUSTOM MDBLIST ROWS</span><h2>Connect MDBList for your own lists</h2><p>Swoop's built-in Top 100 and Trending rows are already automatic. An MDBList key on this device is only needed for custom personal MDBList rows.</p></div><button class="btn accent" data-modal="homeRows">Set up Custom Lists</button></section>`:'';
  const status=discoveryRefreshing?'Refreshing Swoop discovery…':discoveryMessage||'Trending refreshes about every 90 minutes · Top 100 every 4 hours';
  return `<main class="home-main">${hero(feature,providerName,{total:heroPool.length,index:heroRotationIndex})}<div class="content home-content"><div class="library-strip home-library-strip"><div><span class="library-dot"></span><strong>${state.catalog.length?esc(providerName):'Demo Library'}</strong><span>${homeCounts.live.toLocaleString()} live · ${homeCounts.movie.toLocaleString()} movies · ${homeCounts.series.toLocaleString()} shows</span></div><div class="home-library-actions"><span class="discovery-status ${discoveryRefreshing?'busy':''}">${esc(status)}</span><button class="library-manage" data-modal="homeRows">☰ Customize Home</button><button class="library-manage" data-modal="provider">${state.catalog.length?'Providers':'Connect Provider'} →</button></div></div>
    ${discoveryNote}${rendered||`<section class="web-discovery-callout"><div><span class="eyebrow">YOUR HOME</span><h2>Choose what Swoop shows here</h2><p>Select Top 100, Trending, Live TV, genres and more. You can change the row order any time.</p></div><button class="btn accent" data-modal="homeRows">Customize Home</button></section>`}
  </div></main>`;
}
function page(kind,title){
  ...
  const loading=nativeCatalogMode&&nativeCache.loading&&!arr.length?`<div class="native-query-loading"><div><span class="provider-spinner"></span><strong>Loading ${esc(title)} from the local catalogue…</strong><div class="activity-progress indeterminate"><b></b></div><small>Still working — Swoop has not frozen.</small></div></div>`:'';
...
function livePage(){
  ...
  const loading=nativeCatalogMode&&nativeCache.loading&&!all.length?`<div class="native-query-loading"><div><span class="provider-spinner"></span><strong>Loading channels from the local catalogue…</strong><div class="activity-progress indeterminate"><b></b></div><small>Still working — Swoop has not frozen.</small></div></div>`:'';
...
function restoringPage(){
  return `<main class="page restoring-page"><div class="restore-card"><div class="provider-spinner" aria-hidden="true"></div><div class="eyebrow">RESTORING SWOOP</div><h1>Loading your TV library…</h1><p id="restoreProgressText">Your profile is ready. Swoop is loading the large provider catalog in the background without blocking the profile screen.</p><div class="restore-progress"><div><span id="restoreProgressCount">Preparing saved library…</span><strong id="restoreProgressPercent">4%</strong></div><i><b id="restoreProgressBar" style="width:4%"></b></i></div><small>Large libraries are restored in smaller chunks so Windows can keep the app responsive.</small></div></main>`;
}
...
function render(){
  ...
  $app.innerHTML=`<div class="app-shell">${shellNav}${body}${modal?modalHtml():''}${!profilePickerOpen&&sourceChoiceItem?sourceChoiceHtml():''}${!profilePickerOpen&&playerItem&&!playerUiHidden?playerHtml():''}${!profilePickerOpen&&!detailRoute?backgroundLiveBar():''}${!profilePickerOpen&&trailerKey?trailerHtml():''}${taskProgressHtml()}</div>`;
...
function taskProgressHtml(){
  if(!longTask)return'';const pct=longTask.progress!==null&&longTask.progress!==undefined&&Number.isFinite(Number(longTask.progress))?Math.max(0,Math.min(100,Number(longTask.progress))):null,finished=longTask.status==='success'||longTask.status==='error',label=longTask.status==='success'?'COMPLETE':longTask.status==='error'?'NEEDS ATTENTION':'SWOOP IS WORKING';
  return `<aside class="task-progress-hud ${finished?'finished':''} ${longTask.status==='error'?'error':''}" data-task-progress-hud role="status" aria-live="polite" aria-busy="${finished?'false':'true'}"><div class="task-progress-copy"><span>${label}</span><strong>${esc(longTask.title||'Working…')}</strong><small>${esc(longTask.detail||'Please keep Swoop open while this finishes.')}</small></div><b class="task-progress-value">${pct===null?'WORKING':`${Math.round(pct)}%`}</b><div class="task-progress-track ${pct===null?'indeterminate':''}"><i${pct===null?'':` style="width:${pct}%"`}></i></div><div class="task-progress-foot"><span>${esc(longTaskElapsedLabel())}</span><strong>${finished?(longTask.status==='success'?'Finished successfully':'The task stopped before finishing'):'Still running — Swoop has not frozen.'}</strong></div></aside>`;
}
...
function taskProgressStart({title='Working…',detail='Please keep Swoop open while this finishes.',progress=null}={}){
...
async function openDetail(item){
  if(!item)return;
  if(!detailItem){detailReturnScroll=window.scrollY||document.documentElement.scrollTop||0;detailScrollTop=0;suspendBaseViewForDetail();}
...
async function refreshProvider(id,{quiet=false,manageTask=true,onProgress=null}={}){
  ...
  try{
    ...
    if(p.type==='xtream'){
      ...
      const url=cfg.url||p.url;report(10,'Downloading the provider playlist…');const text=NATIVE_WINDOWS?await nativeFetchText(url):await (await fetch(url,{cache:'no-store'})).text();report(58,'Playlist downloaded. Parsing channels, movies and shows…');resultItems=parseM3U(text,p.id);counts={live:resultItems.filter(x=>x.kind==='live').length,movie:resultItems.filter(x=>x.kind==='movie').length,series:resultItems.filter(x=>x.kind==='series').length};report(75,`${resultItems.length.toLocaleString()} items parsed. Preparing Swoop library…`);
    }
    if(!resultItems.length)throw new Error('Provider returned an empty catalog.');
    replaceProviderCatalog(p.id,resultItems);report(81,'Updating your unified Swoop library…');if(NATIVE_WINDOWS){await nativeCatalogReplaceProvider(p.id,resultItems,{onProgress:info=>{const pct=82+Math.round((Number(info.loaded||0)/Math.max(1,Number(info.total||1)))*14);report(Math.min(96,pct),`Indexing ${Number(info.loaded||0).toLocaleString()} of ${Number(info.total||0).toLocaleString()} items into the local database…`)}});report(97,'Finishing local library indexes…');await activateNativeCatalogIfAvailable();}Object.assign(p,{lastRefreshed:Date.now(),lastError:'',counts});
