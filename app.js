...
function nav(){
  ...
  return `<header class="topbar"><button class="brand" data-page="home" aria-label="Swoop TV Home"><i class="brand-mark">S</i><span>SWOOP TV</span><b>TV</b></button>
...
function homeRowMarkup(def){
  ...
  if(!data.length){
    if(def.web)return `<section class="section swoop-render-section ${def.poster?'poster-section':'landscape-section'} ${def.ranked?'ranked-section':''} ${PINNED_HOME_ROWS.includes(def.id)?'home-priority-row':''}" data-home-row-mounted="${esc(def.id)}"><div class="section-head"><div><h2>${esc(def.label)}</h2><span class="section-meta">${esc(discoveryMeta(def.id,data))}</span></div><span class="rail-arrow">›</span></div><div class="lazy-row-skeleton poster-skeleton">${Array.from({length:6},()=>'<i></i>').join('')}</div></section>`;
    return'';
  }
  const limit=String(def.id).startsWith('top20-')?HOME_RANKED_ROW_LIMIT:HOME_STANDARD_ROW_LIMIT;
...
function page(kind,title){
  ...
  const loading=nativeCatalogMode&&nativeCache.loading&&!arr.length?`<div class="native-query-loading"><div><span class="provider-spinner"></span><strong>Loading ${esc(title)} from the local catalogue…</strong><div class="activity-progress indeterminate"><b></b></div><small>Still working — Swoop TV has not frozen.</small></div></div>`:'';
...
function livePage(){
  ...
  const loading=nativeCatalogMode&&nativeCache.loading&&!all.length?`<div class="native-query-loading"><div><span class="provider-spinner"></span><strong>Loading channels from the local catalogue…</strong><div class="activity-progress indeterminate"><b></b></div><small>Still working — Swoop TV has not frozen.</small></div></div>`:'';
...
function guideCategories(){
  ...
  if(nativeCatalogMode){
    const cats=(nativeCategoryCache.live||[]).filter(x=>x?.name&&Number(x.count)>0).map(x=>({name:String(x.name),count:Number(x.count||0)}));
    return [{name:'',label:'All Channels',count:total},...cats];
  }
  const counts=new Map();for(const ch of providerFiltered(items('live'))){const group=String(ch.group||'Uncategorised').trim()||'Uncategorised';counts.set(group,(counts.get(group)||0)+1)}
  return [{name:'',label:'All Channels',count:total},...[...counts].sort((a,b)=>a[0].localeCompare(b[0])).map(([name,count])=>({name,count}))];
}
...
function guidePage(){
  ...
  return `<main class="page guide-page"><section class="guide-hero"><div><div class="eyebrow">LIVE TV</div><h1>TV Guide</h1><p>${esc(providerGuide)} · ${totalAll.toLocaleString()} channels · category-based loading</p></div><div class="guide-now"><span>NOW</span><strong>${new Date().toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})}</strong></div></section>
  ...
  <div class="guide-browser"><aside class="guide-categories"><div class="guide-categories-head"><span class="eyebrow">CHANNEL GROUPS</span><h2>Categories</h2><small>Only the selected category is loaded into the guide.</small></div><div class="guide-category-list">${guideCategoryButtons()}</div></aside>
...
async function m3uGuideText(provider){
  ...
  const text=NATIVE_WINDOWS?await nativeFetchText(provider.epgUrl):await (await fetch(provider.epgUrl,{cache:'no-store'})).text();m3uGuideTextCache.set(provider.id,{loadedAt:Date.now(),text});return text;
}
...
function updateRestoreProgress(info={}){
  ...
  if(text)text.textContent=info.phase==='sqlite'?'Optimizing your library into Swoop TV’s local SQLite catalogue. This one-time migration makes future launches, browsing and search much lighter.':info.phase==='legacy'?'Upgrading your older Swoop TV library storage. This happens once and is being processed away from the main screen.':info.phase==='finishing'?'Finishing your library and provider indexes…':'Loading channels, movies and TV shows in responsive chunks…';
}
...
function render(){
  ...
  hydrateArtwork();
  if(!profilePickerOpen&&!mediaRoute&&state.page==='guide')setTimeout(loadGuideEpg,0);
  ...
  if(!profilePickerOpen&&!mediaRoute&&nativeCatalogMode){if(state.page==='movies')scheduleNativePage('movie');if(state.page==='series')scheduleNativePage('series');if(state.page==='live')scheduleNativePage('live');}
...
function taskProgressHtml(){
  ...
  return `<aside class="task-progress-hud ${finished?'finished':''} ${longTask.status==='error'?'error':''}" data-task-progress-hud role="status" aria-live="polite" aria-busy="${finished?'false':'true'}"><div class="task-progress-copy"><span>${label}</span><strong>${esc(longTask.title||'Working…')}</strong><small>${esc(longTask.detail||'Please keep Swoop TV open while this finishes.')}</small></div><b class="task-progress-value">${pct===null?'WORKING':`${Math.round(pct)}%`}</b><div class="task-progress-track ${pct===null?'indeterminate':''}"><i${pct===null?'':` style="width:${pct}%"`}></i></div><div class="task-progress-foot"><span>${esc(longTaskElapsedLabel())}</span><strong>${finished?(longTask.status==='success'?'Finished successfully':'The task stopped before finishing'):'Still running — Swoop TV has not frozen.'}</strong></div></aside>`;
}
...
function patchProviderRefreshCard(p){
  if(!p)return;const card=[...document.querySelectorAll('[data-provider-card]')].find(x=>x.dataset.providerCard===p.id);if(!card)return;const progress=card.querySelector('[data-provider-inline-progress]'),bar=card.querySelector('[data-provider-progress-bar]'),percent=card.querySelector('[data-provider-progress-percent]'),detail=card.querySelector('[data-provider-progress-detail]'),status=card.querySelector('[data-provider-status-copy]'),health=card.querySelector('[data-provider-health]'),refresh=card.querySelector('[data-provider-refresh]');const pct=Math.max(0,Math.min(100,Number(p.refreshProgress||0)));if(progress)progress.hidden=p.status!=='refreshing';if(bar)bar.style.width=`${pct}%`;if(percent)percent.textContent=`${Math.round(pct)}%`;if(detail)detail.textContent=p.refreshDetail||'Updating provider catalogue…';if(status)status.textContent=providerStatusCopy(p);if(health){health.textContent=p.status==='error'?'Needs attention':p.status==='refreshing'?'Refreshing…':p.enabled===false?'Disabled':'Connected';health.className=`provider-health ${p.status==='error'?'error':p.enabled===false?'off':'ok'}`;}if(refresh){refresh.disabled=p.status==='refreshing';refresh.textContent=p.status==='refreshing'?'↻ Refreshing…':'↻ Refresh';}}
...
function detailHtml(){
  ...
  return `<main class="detail-overlay detail-route" aria-label="${esc(meta.title)}"><button class="detail-close" data-detail-close aria-label="Back">←</button><div class="detail-scroll"><section class="detail-hero ${hasCinematicBackdrop?'has-backdrop':'poster-fallback'}"><div class="detail-media"><div class="detail-fallback" style="--detail-fallback:${detailItem.demoColor||'linear-gradient(135deg,#151b2a,#050609)'}"></div>${backdrop?`<img class="detail-backdrop" data-swoop-art="${esc(backdrop)}" alt="">`:''}</div><div class="detail-vignette"></div><div class="detail-copy"><div class="eyebrow">${esc(kindLabel(detailItem).toUpperCase())}</div><div class="detail-title-slot" data-detail-title><h2 class="detail-title-text">${esc(meta.title)}</h2>${meta.titleLogo?`<img class="detail-title-logo" data-swoop-art="${esc(meta.titleLogo)}" alt="${esc(meta.title)}">`:''}</div><div class="detail-meta">${[meta.year,meta.rating?`★ ${meta.rating}`:'',meta.age,detailItem.group].filter(Boolean).map(x=>`<span>${esc(x)}</span>`).join('')}</div><p>${esc(meta.plot||`Available from ${providerSummaryName()}.`)}</p><div class="cta-row">${primary}${trailerButton}<button class="btn secondary detail-list ${saved?'saved':''}" data-toggle-list="${esc(detailItem.id)}"><span>${saved?'✓':'＋'}</span> ${saved?'In My List':'My List'}</button>${watchedButton}</div></div>${meta.cover&&!hasCinematicBackdrop?`<img class="detail-poster" data-swoop-art="${esc(meta.cover)}" alt="">`:''}</section>
...
function patchDetailHeroFromState(){
  ...
  if(slot&&meta.titleLogo){const current=[...slot.querySelectorAll('.detail-title-logo')].find(x=>x.dataset.swoopArt===meta.titleLogo),old=[...slot.querySelectorAll('.detail-title-logo')].find(x=>x.dataset.swoopArt!==meta.titleLogo);if(!current){const logo=document.createElement('img');logo.className='detail-title-logo detail-title-logo-next';logo.dataset.swoopArt=meta.titleLogo;logo.alt=title;logo.addEventListener('load',()=>setTimeout(()=>{old?.classList.add('detail-title-logo-retiring');setTimeout(()=>old?.remove(),260);logo.classList.remove('detail-title-logo-next')},180),{once:true});slot.appendChild(logo);loadArtwork(logo)}}
...
function bindPatchedDetail(root=document){
  bindDynamicCards(root);bindPersonLinks(root);
...
async function openDetail(item){
  ...
  // Route immediately. The detail shell renders once; background metadata/source work patches it in place
  // so buttons are never replaced underneath a click and loaded artwork never flashes back to black.
  ...
  if(['movie','series'].includes(item.kind))setTimeout(async()=>{await enrichItemMetadata(item,{rerender:false});if(detailItem?.id===openingId)patchDetailFromState({sections:true,controls:false})},0);
  let resolved=item;
  ...
  if(detailPayload||resolved.source!=='xtream'||!['movie','series'].includes(resolved.kind)){detailLoading=false;patchDetailFromState({sections:true,controls:resolved.kind==='series'});return;}
...
async function play(item,{sourceSelected=false}={}){
  ...
  if(!item.streamUrl){
    toast(item.source==='demo'?'Demo item — connect your provider for playback.':item.kind==='series'?'Open the series to choose an episode.':'Swoop TV could not resolve a playable source for this title.');
...
function replaceProviderCatalog(providerId,newItems=[]){state.catalog=[...state.catalog.filter(x=>x.providerId!==providerId),...newItems];resetMovieStackIndex();syncProviderCounts();state.webDiscovery={};epgCache.clear();m3uGuideTextCache.delete(providerId);guideChannelCache.key='';guideChannelCache.items=[];detailCache.clear()}
function providerStatusCopy(p){if(p.status==='error')return p.lastError||'Provider refresh failed';if(p.status==='refreshing')return`${Math.round(Math.max(0,Math.min(100,Number(p.refreshProgress||0))))}% · ${p.refreshDetail||'Updating provider catalogue…'}`;if(p.enabled===false)return'Provider is disabled';return p.lastRefreshed?`Last refreshed ${new Date(p.lastRefreshed).toLocaleString()}`:'Connected'}
async function refreshProvider(id,{quiet=false,manageTask=true,onProgress=null}={}){
  const p=providerById(id);if(!p)return false;const cfg=providerConfigById(id)||{};p.status='refreshing';p.lastError='';p.refreshProgress=2;p.refreshDetail='Starting provider refresh…';if(!quiet)render();else patchProviderRefreshCard(p);if(manageTask)taskProgressStart({title:`Refreshing ${p.name||'TV provider'}…`,detail:'Starting provider refresh…',progress:2});
  ...
  try{
    ...
    if(p.type==='xtream'){
      ...
      report(8,'Contacting provider and checking the saved login…');const auth=await testXtream(cfg);if(String(auth?.user_info?.auth)==='0')throw new Error('Xtream account is not authorised.');report(17,'Login verified. Loading Live TV, Movies and TV Shows…');
      const completed=new Set(),result=await importXtream(cfg,p.id,info=>{if(!info?.section)return;completed.add(info.section);const loaded=[...completed].map(section=>`${section==='live'?'Live TV':section==='movie'?'Movies':'TV Shows'} ✓`).join(' · '),pct=17+completed.size*18;report(Math.min(71,pct),`${loaded}. ${completed.size<3?'Other sections are still downloading…':'Provider catalogue downloaded.'}`)});resultItems=result.items;counts=result.counts||counts;report(76,`${resultItems.length.toLocaleString()} provider items received. Preparing Swoop TV library…`);
    }else{
      ...
      const url=cfg.url||p.url;report(10,'Downloading the provider playlist…');const text=NATIVE_WINDOWS?await nativeFetchText(url):await (await fetch(url,{cache:'no-store'})).text();report(58,'Playlist downloaded. Parsing channels, movies and shows…');resultItems=parseM3U(text,p.id);counts={live:resultItems.filter(x=>x.kind==='live').length,movie:resultItems.filter(x=>x.kind==='movie').length,series:resultItems.filter(x=>x.kind==='series').length};report(75,`${resultItems.length.toLocaleString()} items parsed. Preparing Swoop TV library…`);
    }
    if(!resultItems.length)throw new Error('Provider returned an empty catalog.');
    replaceProviderCatalog(p.id,resultItems);report(81,'Updating your unified Swoop TV library…');if(NATIVE_WINDOWS){await nativeCatalogReplaceProvider(p.id,resultItems,{onProgress:info=>{const pct=82+Math.round((Number(info.loaded||0)/Math.max(1,Number(info.total||1)))*14);report(Math.min(96,pct),`Indexing ${Number(info.loaded||0).toLocaleString()} of ${Number(info.total||0).toLocaleString()} items into the local database…`)}});report(97,'Finishing local library indexes…');await activateNativeCatalogIfAvailable();}Object.assign(p,{lastRefreshed:Date.now(),lastError:'',counts});
...
function bind(){
  ...
  bindDynamicCards(document);bindPersonLinks(document);
