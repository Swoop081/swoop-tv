...
function settingsPage(){
  ...
  <section class="setting-card performance-setting-card"><div class="setting-icon">⚡</div><div class="setting-main"><h3>Performance</h3><p>${performanceLabel()}. Auto mode reduces off-screen rendering and background metadata work when Swoop TV detects a very large library.</p><div class="cta-row"><button class="btn ${state.settings.performanceMode!=='cinematic'?'accent':'secondary'}" data-performance-mode="auto">Auto / Recommended</button><button class="btn ${state.settings.performanceMode==='cinematic'?'accent':'secondary'}" data-performance-mode="cinematic">Full Cinematic</button></div><small>${catalogLogicalTotal().toLocaleString()} enabled library items · ${nativeCatalogMode?'SQLite query mode active · ':''}large-library optimization starts at ${LARGE_LIBRARY_THRESHOLD.toLocaleString()}.</small></div></section>
...
function replaceProviderCatalog(providerId,newItems=[]){state.catalog=[...state.catalog.filter(x=>x.providerId!==providerId),...newItems];resetMovieStackIndex();syncProviderCounts();state.webDiscovery={};epgCache.clear();m3uGuideTextCache.delete(providerId);xtreamGuideTextCache.delete(providerId);guideProviderCategoryCache.key='';guideProviderCategoryCache.names=[];guideChannelCache.key='';guideChannelCache.items=[];detailCache.clear()}
