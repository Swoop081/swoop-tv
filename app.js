...
async function refreshDiscoveryRows(force=false){
  ...
  try{
    ...
    persist();discoveryMessage='Discovery updated';
...
function providerModal(){
  ...
  return `<div class="modal-backdrop" data-close-modal><div class="modal provider-modal" data-modal-card><div class="modal-head provider-modal-head"><div><div class="eyebrow">TV PROVIDER</div><h2>${state.provider?'Manage Provider':'Add Provider'}</h2><p>Choose how your TV service was supplied. Swoop only shows the fields required for that connection type.</p></div><button class="icon-btn" data-close aria-label="Close">✕</button></div><div class="modal-body provider-modal-body">${connected}<div id="providerSetup"><div class="provider-methods" aria-label="Provider type"><button type="button" class="provider-method active" data-provider-tab="xtream"><span class="provider-method-icon">X</span><span><strong>Xtream Codes</strong><small>Server URL + username + password</small></span><span class="provider-method-check">✓</span></button><button type="button" class="provider-method" data-provider-tab="m3u"><span class="provider-method-icon">M3U</span><span><strong>M3U Playlist</strong><small>Playlist URL or local M3U file</small></span><span class="provider-method-check">✓</span></button></div>
    <form id="xtreamForm" class="provider-form"><div class="provider-form-intro"><div><div class="eyebrow">XTREAM CODES</div><h3>Connect your TV service</h3><p>Enter the same Xtream details you use in another IPTV player.</p></div><span class="provider-badge">Recommended</span></div><div class="field"><label>Provider name</label><input name="name" value="${esc(state.provider?.type==='xtream'?state.provider?.name||'My TV':'My TV')}" placeholder="My TV" required></div><div class="field"><label>Server URL</label><input name="server" type="url" value="${esc(state.provider?.type==='xtream'?state.provider?.server||'':'')}" placeholder="http://provider.example:port" required></div><div class="split"><div class="field"><label>Username</label><input name="username" value="${esc(state.provider?.type==='xtream'?state.provider?.username||'':'')}" autocomplete="username" required></div><div class="field"><label>Password</label><input name="password" type="password" value="${esc(state.provider?.type==='xtream'?state.provider?.password||'':'')}" autocomplete="current-password" required></div></div>${helper}<label class="remember-row provider-remember"><input type="checkbox" name="remember" ${state.provider?.username?'checked':''}><span><strong>Remember login on this device</strong><small>Credentials are stored locally and are not encrypted by the browser.</small></span></label><button class="btn accent provider-primary" type="submit"><span>Connect Xtream</span><span>→</span></button></form>
    <form id="m3uForm" class="provider-form" hidden><div class="provider-form-intro"><div><div class="eyebrow">M3U PLAYLIST</div><h3>Import your playlist</h3><p>Use either a playlist URL or a local M3U/M3U8 file.</p></div></div><div class="field"><label>Provider name</label><input name="name" value="${esc(state.provider?.type==='m3u'?state.provider?.name||'My TV':'My TV')}" placeholder="My TV" required></div><div class="field"><label>M3U playlist URL</label><input name="url" type="url" placeholder="http://provider.example/get.php?... "></div><div class="provider-or"><span>or</span></div><div class="field"><label>Choose M3U file</label><input name="file" type="file" accept=".m3u,.m3u8,text/plain,application/x-mpegURL"></div><div class="field"><label>TV guide / XMLTV URL <span class="optional">Optional</span></label><input name="epgUrl" type="url" value="${esc(state.provider?.type==='m3u'?state.provider?.epgUrl||'':'')}" placeholder="http://provider.example/epg.xml"></div><div class="provider-note"><div class="provider-note-icon">i</div><div><strong>${NATIVE_WINDOWS?'Windows import ready':'Playlist import'}</strong><span>${NATIVE_WINDOWS?'The Windows bridge can fetch HTTP or HTTPS playlist URLs directly.':'Local files work immediately. URL imports require the playlist server to allow browser requests.'}</span></div></div><button class="btn accent provider-primary" type="submit"><span>Import M3U</span><span>→</span></button></form></div>
...
function detailHtml(){
  ...
  const meta=detailMeta(detailItem,detailPayload||{}),saved=isInMyList(detailItem),backdrop=meta.backdrop||meta.cover||detailItem.logo;
  ...
  return `<div class="detail-overlay" role="dialog" aria-modal="true" aria-label="${esc(meta.title)}"><div class="detail-scroll"><section class="detail-hero"><div class="detail-media">${backdrop?`<img class="detail-backdrop" data-swoop-art="${esc(backdrop)}" alt="">`:''}<div class="detail-fallback" style="--detail-bg:${detailItem.demoColor||'linear-gradient(135deg,#252539,#090909)'}"></div></div><div class="detail-vignette"></div><button class="detail-close" data-detail-close aria-label="Close">←</button><div class="detail-copy"><div class="detail-kicker">${esc(kindLabel(detailItem).toUpperCase())}</div><h2>${esc(meta.title)}</h2><div class="detail-meta">${[meta.year,meta.rating?`★ ${meta.rating}`:'',meta.age,detailItem.group].filter(Boolean).map(x=>`<span>${esc(x)}</span>`).join('')}</div><p>${esc(meta.plot||`Available from ${state.provider?.name||'your connected provider'}.`)}</p><div class="cta-row">${primary}<button class="btn secondary detail-list ${saved?'saved':''}" data-toggle-list="${esc(detailItem.id)}"><span>${saved?'✓':'＋'}</span> ${saved?'In My List':'My List'}</button></div></div>${meta.cover?`<img class="detail-poster" data-swoop-art="${esc(meta.cover)}" alt="">`:''}</section>
...
async function openDetail(item){
  if(!item)return;detailItem=item;detailSeason='';detailError='';detailPayload=detailCache.get(item.id)||null;detailLoading=false;render();
...
function persist(){saveState({...state,page:'home',favourites:state.myList})}
