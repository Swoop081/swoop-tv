...
import {loadState, loadBulkState, saveState, saveBulkState, loadProviderProfile, saveProviderProfile, clearProviderProfile, loadProviderProfiles, saveProviderProfiles, clearProviderProfiles, clearState} from './src/storage.js';
import {demoCatalog} from './src/demo.js';

const NATIVE_WINDOWS=isNativeWindows();
const DEFAULT_HOME_ROWS=['continue','recently-watched','recommended','recent-live','mylist','top20-movies','top20-shows','trending-movies','trending-shows','live-now','new-movies','new-shows','action-movies','comedy-movies','drama-shows'];
const DEFAULT_STATE={page:'home',catalog:[],provider:null,providers:[],myList:[],favourites:[],liveFavourites:[],continueWatching:[],watchHistory:[],recentLive:[],profiles:[],activeProfileId:'',mdblistRows:[],webDiscovery:{},metadataCache:{},settings:{mdblistApiKey:'',xtreamRelayUrl:'',xtreamRelayToken:'',metadataServiceUrl:'',themeId:'chill',backgroundColor:'#050505',backgroundOverride:false,movieSourcePreferences:{},homeRows:[...DEFAULT_HOME_ROWS],smartHomeOrder:true}};
...
let guideLimit=24,liveCategory='',providerFilter='all';
...
function providerCatalogCounts(id){const list=state.catalog.filter(x=>x.providerId===id);return {live:list.filter(x=>x.kind==='live').length,movie:list.filter(x=>x.kind==='movie').length,series:list.filter(x=>x.kind==='series').length,total:list.length}}
...
let storageRestoring=Boolean(state.providers.length && !state.catalog.length);
...
function activeCatalog(){let base=state.catalog.length?state.catalog:demoCatalog;if(state.catalog.length&&state.providers.length){const enabled=new Set(enabledProviders().map(p=>p.id));base=base.filter(item=>!item.providerId||enabled.has(item.providerId));}const profile=activeProfile();return profile?.kids?base.filter(item=>profileAllowsMedia(profile,item,state.metadataCache?.[item.id]||{})):base}
let movieStackCatalogRef=null,movieStackIndex=null,liveStackCatalogRef=null,liveStackIndex=null;
function getMovieStackIndex(){const catalog=activeCatalog();if(movieStackCatalogRef!==catalog||!movieStackIndex){movieStackCatalogRef=catalog;movieStackIndex=buildMovieStackIndex(catalog,providerPriorityMap())}return movieStackIndex}
...
function getLiveStackIndex(){const catalog=activeCatalog();if(liveStackCatalogRef!==catalog||!liveStackIndex){liveStackCatalogRef=catalog;liveStackIndex=buildLiveStackIndex(catalog,providerPriorityMap())}return liveStackIndex}
function resetMovieStackIndex(){movieStackCatalogRef=null;movieStackIndex=null;liveStackCatalogRef=null;liveStackIndex=null}
...
function logicalItemIds(item){return item?[item.id,...(Array.isArray(item.sources)?item.sources.map(x=>x.id):[])]:[]}
...
function savedItem(id){const stack=getMovieStackIndex(),live=getLiveStackIndex();return stack.byStackId.get(id)||stack.bySourceId.get(id)||live.byStackId?.get(id)||live.bySourceId?.get(id)||activeCatalog().find(x=>x.id===id)||detailEpisodeItems.get(id)||state.continueWatching.find(x=>x?.id===id)?.item||null}
...
function scheduleMetadataEnrichment(){
  ...
  for(const watched of watchHistoryItems().slice(0,6)){const source=watched.kind==='episode'?(savedItem(watched.parentSeriesId)||watched):watched;queue.push(source)}
...
function cachedWebRowItems(id){const cache=state.webDiscovery?.[id];return collapseMovieSources((cache?.itemIds||[]).map(savedItem).filter(Boolean),activeCatalog())}
...
async function fetchBuiltInDiscovery(id,apiKey){
  ...
  if(id==='top20-movies'){
    let primary=[];try{const payload=await getMDBListOfficialItems({apiKey,slug:'movies/popular'});primary=matchMDBListToCatalog(payload,activeCatalog(),{limit:20,mediaType:'movie'})}catch{}
    if(primary.length<20){const chart=await getMDBListStreamingChart({apiKey,mediaType:'movie'});primary=mergeUnique(primary,matchMDBListToCatalog(chart,activeCatalog(),{limit:20,mediaType:'movie'}));}
    return primary;
  }
  if(id==='top20-shows'){
    let primary=[];try{const payload=await getMDBListOfficialItems({apiKey,slug:'shows/popular'});primary=matchMDBListToCatalog(payload,activeCatalog(),{limit:20,mediaType:'show'})}catch{}
    if(primary.length<20){const chart=await getMDBListStreamingChart({apiKey,mediaType:'show'});primary=mergeUnique(primary,matchMDBListToCatalog(chart,activeCatalog(),{limit:20,mediaType:'show'}));}
    return primary;
  }
  if(id==='trending-movies'){const payload=await getMDBListStreamingChart({apiKey,mediaType:'movie'});return matchMDBListToCatalog(payload,activeCatalog(),{limit:20,mediaType:'movie'});}
  if(id==='trending-shows'){const payload=await getMDBListStreamingChart({apiKey,mediaType:'show'});return matchMDBListToCatalog(payload,activeCatalog(),{limit:20,mediaType:'show'});}
  return[];
}
async function refreshDiscoveryRows(force=false){
  ...
  try{
    ...
    for(const row of staleCustom){try{const payload=await getMDBListItems({apiKey,listId:row.source.listId,username:row.source.username,listName:row.source.listName});row.items=matchMDBListToCatalog(payload,activeCatalog());row.updatedAt=Date.now();row.error='';}catch(err){row.updatedAt=Date.now();row.error=err.message||String(err);}}
...
async function switchProfile(id,{skipPin=false}={}){
  ...
  if(target.id===state.activeProfileId){profilePickerOpen=false;modal=null;await persist();render();return}
  if(playerItem)await stopPlayback(true);
  syncActiveProfileFromState();state.activeProfileId=target.id;applyProfileToState(target);profilePickerOpen=false;modal=null;detailItem=null;sourceChoiceItem=null;heroRotationIndex=0;await persist();render();toast(`Switched to ${target.name}`);
}
function nav(){
...
function home(){
  const cat=activeCatalog(),live=items('live'),movies=items('movie'),shows=cat.filter(x=>x.kind==='series');
...
function page(kind,title){
  const arr=providerFiltered(items(kind)),limit=viewLimits[kind]||120,shown=arr.slice(0,limit),providerName=providerSummaryName(),providerPills=providerFilterOptions();
  ...
  return `<main class="page cinematic-page"><section class="page-hero ${kind==='live'?'live-page-hero':''}">${leadArt}<div class="page-hero-shade"></div><div class="page-hero-copy"><div class="eyebrow">${kind==='live'?'WATCH NOW':kind==='movie'?'ON DEMAND':'BINGE-WORTHY'}</div><h1>${esc(title)}</h1><p>${state.catalog.length?`${arr.length.toLocaleString()} ${kind==='live'?'channels':kind==='movie'?'movies':'series'} from ${esc(providerName)}.`:'Demo content — connect a provider to populate your library.'}</p><div class="cta-row">${leadAction}${kind==='live'?'<button class="btn secondary" data-page="guide">▤ Open TV Guide</button>':''}</div></div></section>
    <div class="page-content"><div class="provider-filter-pills"><button class="${providerFilter==='all'?'active':''}" data-provider-filter="all">All Providers</button>${providerPills.map(p=>`<button class="${providerFilter===p.id?'active':''}" data-provider-filter="${esc(p.id)}">${esc(p.name)}</button>`).join('')}</div><div class="page-toolbar"><div class="category-pills">${groups.map(g=>`<button data-search-term="${esc(g)}">${esc(g)}</button>`).join('')}</div><button class="btn secondary compact-btn" data-modal="provider">＋ Provider</button></div>${arr.length?`<div class="content-grid ${kind==='live'?'live-content-grid':'poster-content-grid'}">${cards}</div>${shown.length<arr.length?`<div class="load-more-wrap"><button class="btn secondary" data-load-more="${kind}">Load more · showing ${shown.length.toLocaleString()} of ${arr.length.toLocaleString()}</button></div>`:''}`:empty('No content yet','Connect a TV provider to populate this section.')}</div></main>`;
}
function livePage(){
  const all=providerFiltered(items('live')),providerName=providerSummaryName(),providerPills=providerFilterOptions();
  const byId=new Map(all.map(x=>[x.id,x]));
  const favourites=state.liveFavourites.map(id=>byId.get(id)).filter(Boolean);
  const recent=state.recentLive.map(id=>byId.get(id)).filter(Boolean).slice(0,16);
  const groups=[...new Set(all.map(x=>x.group).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  if(liveCategory&&!groups.includes(liveCategory))liveCategory='';
  const filtered=liveCategory?all.filter(x=>x.group===liveCategory):all;
  const shown=filtered.slice(0,viewLimits.live||180);
  ...
  return `<main class="page live-hub-page"><section class="live-hub-hero"><div class="live-hub-backdrop">${art}</div><div class="live-hub-shade"></div><div class="live-hub-copy"><div class="eyebrow">LIVE TV · ${esc(providerName)}</div><h1>${lead?esc(lead.name):'Live TV'}</h1><p>${lead?`Jump straight back into ${esc(lead.name)}, browse favourites, or surf every live channel from one TV-first screen.`:'Connect a TV provider to populate Live TV.'}</p><div class="cta-row">${lead?`<button class="btn play-btn" data-play="${esc(lead.id)}">▶ Watch Live</button><button class="btn secondary" data-live-favourite="${esc(lead.id)}">${leadFav?'★ Favourite':'☆ Add Favourite'}</button>`:''}<button class="btn secondary" data-page="guide">▤ TV Guide</button></div></div><div class="live-hub-stat"><strong>${all.length.toLocaleString()}</strong><span>LIVE CHANNELS</span><small>${favourites.length} favourites · ${recent.length} recent</small></div></section>
  ...
  <section class="live-browser"><div class="section-head live-browser-head"><div><h2>${liveCategory?esc(liveCategory):'All Channels'}</h2><span class="section-meta">${filtered.length.toLocaleString()} channels</span></div><button class="btn secondary compact-btn" data-page="guide">Open Guide</button></div><div class="live-category-pills"><button class="${!liveCategory?'active':''}" data-live-category="">All</button>${groups.map(g=>`<button class="${liveCategory===g?'active':''}" data-live-category="${esc(g)}">${esc(g)}</button>`).join('')}</div>${shown.length?`<div class="content-grid live-content-grid premium-live-grid">${shown.map(x=>card(x,false)).join('')}</div>${shown.length<filtered.length?`<div class="load-more-wrap"><button class="btn secondary" data-load-more="live">Load more · showing ${shown.length.toLocaleString()} of ${filtered.length.toLocaleString()}</button></div>`:''}`:empty('No channels in this category','Choose another Live TV category.')}</section></div></main>`;
}
...
function render(){
  applyTheme();
  let body;if(storageRestoring)body=restoringPage();else if(profilePickerOpen)body=profilePickerPage();else if(state.page==='home')body=home();else if(state.page==='live')body=livePage();else if(state.page==='guide')body=guidePage();else if(state.page==='movies')body=page('movie','Movies');else if(state.page==='series')body=page('series','TV Shows');else if(state.page==='mylist')body=myListPage();else if(state.page==='search')body=searchPage();else body=settingsPage();
  const shellNav=storageRestoring||profilePickerOpen?'':nav();
  $app.innerHTML=`<div class="app-shell">${shellNav}${body}${modal?modalHtml():''}${!profilePickerOpen&&detailItem?detailHtml():''}${!profilePickerOpen&&sourceChoiceItem?sourceChoiceHtml():''}${!profilePickerOpen&&playerItem&&!playerUiHidden?playerHtml():''}${!profilePickerOpen?backgroundLiveBar():''}${!profilePickerOpen&&trailerKey?trailerHtml():''}</div>`;bind();bindHeroControls(document);if(!profilePickerOpen&&state.page==='search')runSearch('');hydrateArtwork();if(!profilePickerOpen&&state.page==='guide')setTimeout(loadGuideEpg,0);if(!profilePickerOpen&&state.page==='home'&&state.catalog.length&&state.settings.mdblistApiKey)setTimeout(()=>refreshDiscoveryRows(false),0);if(!profilePickerOpen&&playerItem?.kind==='live')setTimeout(()=>{loadPlayerNowNext(playerItem);loadLiveMiniGuide(playerItem)},0);if(!profilePickerOpen&&state.catalog.length)setTimeout(scheduleMetadataEnrichment,80);scheduleHeroRotation();
}
...
function recordWatchHistory(item){
  if(!item)return;
  if(item.kind==='live'){
    state.recentLive=[item.id,...state.recentLive.filter(id=>id!==item.id)].slice(0,20);
    return;
  }
  const entry={id:item.id,item:compactMediaSnapshot(item),lastPlayed:Date.now(),selectedSourceId:item._selectedSourceId||''};
  state.watchHistory=state.watchHistory.filter(x=>x.id!==item.id);state.watchHistory.unshift(entry);state.watchHistory=state.watchHistory.slice(0,80);
}
function rememberWatching(item){
  ...
  state.continueWatching=state.continueWatching.filter(x=>x.id!==item.id);state.continueWatching.unshift(entry);state.continueWatching=state.continueWatching.slice(0,40);persist();
}
...
function updateContinueProgress(item,pb,force=false){
  ...
  if(complete){state.continueWatching=state.continueWatching.filter(x=>x.id!==item.id);persist();return}
  ...
  state.continueWatching=state.continueWatching.filter(x=>x.id!==item.id);state.continueWatching.unshift(entry);state.continueWatching=state.continueWatching.slice(0,40);
...
async function play(item,{sourceSelected=false}={}){if(!item)return;if(item.kind==='live'&&Array.isArray(item.sources)&&item.sources.length>1)item=preferredLiveSource(item);if(NATIVE_WINDOWS&&item.kind==='live'&&playerItem?.kind==='live'){await switchLiveChannel(item);return}if(item.kind==='movie'&&Array.isArray(item.sources)&&item.sources.length>1&&!sourceSelected){sourceChoiceItem=item;render();return}if(!item.streamUrl){toast(item.source==='demo'?'Demo item — connect your provider for playback.':item.kind==='series'?'Open the series to choose an episode.':'No playable URL available.');if(item.kind==='series')openDetail(item);return}sourceChoiceItem=null;if(playerItem)await stopPlayback(true);rememberWatching(item);playerItem=item;playerUiHidden=false;render();requestAnimationFrame(()=>startPlayback(item))}
...
function runSearch(q){const out=document.querySelector('#searchResults');if(!out)return;const term=q.trim().toLowerCase();const base=term?activeCatalog().filter(x=>`${x.name} ${x.group} ${x.year||''}`.toLowerCase().includes(term)):activeCatalog();const res=collapseMovieSources(base,activeCatalog()).slice(0,100);out.innerHTML=res.length?res.map(x=>card(x,x.kind!=='live')).join(''):empty('No matches','Try another title, channel or category.');hydrateArtwork(out);bindDynamicCards(out)}
function persist(bulk=false){syncActiveProfileFromState();const snapshot={...state,page:'home',favourites:state.myList};const localOk=saveState(snapshot);return bulk?saveBulkState(snapshot).then(bulkOk=>localOk&&bulkOk):Promise.resolve(localOk)}
...
async function refreshProvider(id,{quiet=false}={}){
  ...
  try{
    ...
    replaceProviderCatalog(p.id,resultItems);Object.assign(p,{status:'connected',lastRefreshed:Date.now(),lastError:'',counts});
    ...
    await persist(true);if(!quiet){render();toast(`${p.name} refreshed`)}return true;
...
async function removeProvider(id){const p=providerById(id);if(!p)return;state.catalog=state.catalog.filter(x=>x.providerId!==id);state.providers=state.providers.filter(x=>x.id!==id);removeSavedProviderCredentials(id);normalizeProviderPriorities();syncProviderCounts();state.webDiscovery={};epgCache.clear();detailCache.clear();await persist(true);render();toast(`${p.name} removed`)}
function toggleProviderEnabled(id){const p=providerById(id);if(!p)return;p.enabled=p.enabled===false;normalizeProviderPriorities();const saved=savedProviderProfiles.find(x=>providerProfileId(x)===id);if(saved){saved.enabled=p.enabled;saveProviderProfiles(savedProviderProfiles)}persist();render();toast(`${p.name} ${p.enabled?'enabled':'disabled'}`)}
...
function bind(){
  ...
  document.querySelectorAll('[data-live-category]').forEach(el=>el.onclick=()=>{liveCategory=el.dataset.liveCategory||'';viewLimits.live=180;render()});
  ...
  document.querySelectorAll('[data-provider-filter]').forEach(el=>el.onclick=()=>{providerFilter=el.dataset.providerFilter||'all';viewLimits.live=180;viewLimits.movie=120;viewLimits.series=120;render()});
