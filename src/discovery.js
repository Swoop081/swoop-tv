...
export async function fetchSwoopDiscovery({settings={},mediaType='movie'}={}){
  ...
  if(!service)throw new Error('Swoop discovery service is not configured.');
  ...
  if(!res.ok){let detail='';try{detail=(await res.json())?.error||''}catch{}throw new Error(detail||`Swoop discovery service returned HTTP ${res.status}.`)}
  return res.json();
}
