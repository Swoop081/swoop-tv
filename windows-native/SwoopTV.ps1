...
function Handle-Request($Request, [string]$MpvPath) {
  ...
  try {
    ...
    if ($path -eq '/native/status') {
      ...
      Send-Json $stream @{ ok=$true; service='Swoop TV Windows Bridge'; version='0.7.13'; platform='windows'; mpvReady=(Test-Path $MpvPath); playing=$playing }
      return
    }
    ...
    if ([IO.Path]::GetFileName($full).ToLowerInvariant() -eq 'index.html') {
      ...
      $bootstrap = "<script>window.__SWOOP_NATIVE__={token:'$SessionToken',version:'0.7.13',platform:'windows'};</script>"
...
Write-Header 'Swoop TV v0.7.13 — Viewport IMDb Rating Hydration'
